---
name: "check-memory-yaml"
description: "Validate YAML memory mirrors for global orchestrator state, per-agent memory and task handoffs."
...
This is the active Claude Code project skill for `/check-memory-yaml`. It is self-contained and delegates directly to scripts, hooks, trailers and generated state.

Run:

```bash
./scripts/check-memory-yaml.sh $ARGUMENTS
```

This is the canonical audit for `orchestrator-state/memory/*.yaml`, `orchestrator-state/tasks/*.yaml`, `orchestrator-state/tasks/handoffs/*.yaml` and `orchestrator-state/agent-memory/*/MEMORY.yaml`.
