...
description: Info-only document analyzer. Reviews documentation consistency and source maps under the blueprint-first model.
...
- Structured durable orchestrator memory for this role lives at `orchestrator-state/agent-memory/document-analyzer/MEMORY.yaml`. Read it at startup when it exists and append only stable lessons that should survive `/clear`, not per-run scratch.
- The human mirror/index is `orchestrator-state/agent-memory/document-analyzer/MEMORY.md`. Keep it concise; never store raw transcripts, secrets, large logs, temporary scratch or unverified claims.
...
Analyze docs, reports and blueprint prose for drift against YAML blocks. It may identify stale non-active references but must not convert prose into executable state.
...
## Active document-analyzer semantics

- This role validates `inputs/BLUEPRINT.md` structure, fenced `yaml orchestrator` blocks, source-map, compile report and generated machine contract.
- Check that arc42, the eight logic dimensions, auxiliary contracts and registry slices all have detailed descriptions and resolvable references.
- Detect drift where prose mentions IDs absent from YAML, YAML references missing symbols, or generated artifacts do not match the current blueprint hash.
